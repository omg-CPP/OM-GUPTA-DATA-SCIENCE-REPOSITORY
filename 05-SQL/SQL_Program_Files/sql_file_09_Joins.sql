...
DROP DATABASE IF EXISTS harryjoins; -- will not be executed as db does not exsist.
...
-- inner join
-- common to both students(id) and marks(student_id) are displayed
SELECT students.name, marks.subject, marks.score FROM students INNER JOIN marks on students.id = marks.student_id;

-- left join
-- all left table entry will come even if data is not present in right table
...
-- cross join -> cartesian product of both table entries (rows)
